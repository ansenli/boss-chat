...
Router.post('/register',function (req,res) {
    const { user,passwd,type } =req.body

    User.findOne({user},function (err,doc) {
        if(doc){
            return res.json({code:1,msg:'用户名密码重复'})
        }
        const userModel = new User({user,type,passwd:md5Pwd(passwd)})
...
function md5Pwd(passwd) {
    const salt ='hsyk_is_good_3.141596@#IUHJh~~'
